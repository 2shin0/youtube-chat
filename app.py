...
# 3data/data_analysis.py 개선 버전
# ✅ 멀티턴 맥락 반영 + ✅ 상태 메시지(placeholder) 표시 정상 작동
...
async def generate_chat_response_async(messages: List[Dict[str, str]], system_prompt: str, message_placeholder):
    ...
    st.session_state.status_text = "🔎 AI가 MCP 서버를 통해 데이터를 가져오는 중입니다..."
    message_placeholder.info(st.session_state.status_text)
    ...
    async with mcp_client:
        ...
        while getattr(response, "function_calls", None):
            tool_results = []
            st.session_state.status_text = f"🔎 MCP Tool {len(response.function_calls)}개 호출 중..."
            message_placeholder.info(st.session_state.status_text)
            ...
            for call in response.function_calls:
                ...
                try:
                    ...
                    st.session_state.status_text = f"✅ Tool `{tool_name}` 호출 완료"
                except Exception as e:
                    tool_output = f"Tool 실행 오류 ({tool_name}): {e}"
                    st.session_state.status_text = f"❌ Tool 호출 실패: {tool_name}"
            ...
            # Tool 결과를 Gemini에 전달하여 후속 응답 생성
