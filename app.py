...
# --- 동기 래퍼 함수: 별도 스레드에서 async 함수 실행 ---
def generate_chat_response(messages: List[Dict[str, str]], system_prompt: str, message_placeholder):
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    return loop.run_until_complete(generate_chat_response_async(messages, system_prompt, message_placeholder))

# --- Streamlit UI 및 세션 관리 (기존 코드는 유지, 사용자 입력 처리 부분만 수정) ---

# 페이지 설정
st.set_page_config(page_title="유튜브 데이터 분석 챗봇", page_icon="📊")

# 대화 기록 초기화 및 현재 세션 설정
...
- 댓글 분석 시 욕설, 인신공격 등은 제외하고 주요 의견의 경향성만 반영합니다.
...
# ThreadPoolExecutor 사용으로 MCP 비동기 호출을 별도 스레드에서 실행
...
if user_input:
    ...
    with st.chat_message("assistant"):
        ...
        # 별도 스레드에서 비동기 함수 실행해 결과 반환
        future = executor.submit(generate_chat_response, current_messages, system_prompt, message_placeholder)
