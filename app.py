...
# --- 사용자 이벤트 루프 캐시 (Streamlit과 충돌 방지용) ---
...
# --- 비동기 챗봇 응답 생성 함수 (Gemini API 동기 호출을 별도 쓰레드로 비동기화) ---
...
def generate_chat_response(messages: List[Dict[str, str]], system_prompt: str, message_placeholder):
    ...
    task = loop.create_task(generate_chat_response_async(messages, system_prompt, message_placeholder))
