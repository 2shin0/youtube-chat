...
# --- FastMCP Tool 호출 함수 ---
async def async_tool_call(client: Client, tool_name: str, tool_args: Dict[str, Any], message_placeholder) -> Any:
    """FastMCP 클라이언트를 전달받아 특정 툴을 호출합니다."""
    try:
        message_placeholder.write(f"🔎 {tool_name} 실행 중...")
        await asyncio.sleep(0.05)
        result = await client.call_tool(tool_name, tool_args)
        output = result.data
        if not isinstance(output, (str, bytes)):
            output_str = json.dumps(output, ensure_ascii=False, indent=2)
            message_placeholder.write(f"✅ {tool_name} 완료\n```json\n{output_str}\n```")
        else:
            message_placeholder.write(f"✅ {tool_name} 완료")
        await asyncio.sleep(0.05)
        return output
    except Exception as e:
        message_placeholder.write(f"❌ {tool_name} 실패: {e}")
        return f"Tool 실행 오류 ({tool_name}): {e}"
...
async def generate_chat_response_async(messages: List[Dict[str, str]], system_prompt: str, message_placeholder):
    ...
    async with mcp_client:
        ...
        while getattr(response, "function_calls", None):
            tool_results = []
            message_placeholder.write(f"🔎 AI가 MCP 서버를 통해 데이터를 가져오는 중 ({len(response.function_calls)}개)...")
            await asyncio.sleep(0.05)
            ...
            for call in response.function_calls:
                ...
                tool_output = await async_tool_call(mcp_client, tool_name, tool_args, message_placeholder)
                tool_results.append(
...
if user_input:
    ...
    with st.chat_message("assistant"):
        ...
        with st.spinner("🤖 YouTube MCP 분석 중... 잠시만 기다려주세요!"):
...
#                 except Exception as e: tool_output = f"Tool 실행 오류 ({tool_name}): {e}" 
